...
import { parseRobots, allowed, parseSitemap, looksLikeJob, pathShape, jobLinks, jobPostings, careerLinks, nextLink, detectPlatform, feedName, repairJson, day, isBoardHost } from '../builder/lib/crawl.mjs';
...
const us = list => list.map(l => [l, placeOf(l, cc).cc]).filter(([, got]) => got !== 'us');
eq(us(['Long Island City, NY, US, 11101', 'Grand Island, NY, US, 14072', 'Holland, MI, US, 49423', 'Poland, ME, US, 04274', 'Macedonia, OH, US, 44056', 'Lake Wales, FL 33859', '739 Paw Paw Drive, Holland MI 49423']), [], 'a postcode after the country or the state does not hide them');
eq(us(['USA - New York - Malta', 'USA - Pennsylvania - North Wales (Upper Gwynedd)', 'USA Wisconsin Luxemburg', 'United States, Ohio, Holland', 'US-NY-Long Island City', 'US.FL.Miami.800 Waterford Way', 'NY - Long Island', 'Ohio - Columbus']), [], 'an address that opens with the United States and a state, or with a state, is American');
eq(us(['Long Island City, New York', 'Staten Island, New York', 'Turkey, Texas', 'Dublin, Ohio', 'Naples, Florida', 'Birmingham, Alabama', 'Providence, Rhode Island, null', 'Rhode Island - Providence', 'Kent Island, MD', "Greece, New York, États-Unis d'Amérique", '1910 SE Federal Hwy Stuart, FL']), [], 'a town named like a European country or city is American when a state ends its place, and "Island" inside a name is not Iceland');
eq(['Gouda, NL, 2801 SC', 'Eindhoven, NB, NL, 5656 AE', 'Madrid, MD, ES, 28050', 'Riga, RI, LV, 1082', 'Balti, MD, 3100', 'Reykjavík, Island', 'LA ROCHELLE, Nouvelle-Aquitaine, FR', 'LA JONQUERA, GIRONA, Spain', 'ID #3gf8b, Lithuania', 'Virginia Water, England', 'Washington, Tyne and Wear, GB', 'London, New York', 'Remote - Australia/Canada/Germany/Hungary/Ireland/Portugal/UK/US', 'US or UK Locations', 'US & Ireland', 'USA (Remote) / Barcelona, Spain', 'European Union, Serbia, Georgia', 'Cambridge, Massachusetts or Ex-US Poland'].map(l => placeOf(l, cc).cc), ['nl', 'nl', 'es', 'lv', 'md', 'is', 'fr', 'es', 'lt', 'gb', 'gb', 'gb', 'gb', 'gb', 'ie', 'es', 'rs', 'pl'], 'and the European look-alikes stay in Europe: Dutch postcodes, region codes, "Island" alone, "LA" and "ID" that are not states, a British Washington, and lists with a European place');
...
}
{
  ...
  eq([tt[0].title, tt[0].company, tt[0].location, tt[0].url, tt[0].posted, tt[0].description, tt[0].remote], ['Battery Cell Engineer', 'Northvolt', 'Skellefteå', 'https://northvolt.teamtailor.com/jobs/1-battery-cell-engineer', '2026-09-01', 'Cells & modules.', false], 'Teamtailor: RSS items with the location in its own tags (hybrid is not remote)');
...

{
  ...
  const twice = parseRobots('User-agent: *\nDisallow: /private/\n\nUser-agent: *\nDisallow: /search\nCrawl-delay: 3\n');
  eq([allowed(twice, '/private/x'), allowed(twice, '/search?q=a'), twice.delay], [false, false, 3], 'two groups for everyone are one (RFC 9309)');
  const ours = parseRobots('User-agent: *\nDisallow: /\n\nUser-agent: ArgusWeb\nAllow: /jobs/\n');
  eq([allowed(ours, '/jobs/1'), allowed(ours, '/about')], [true, true], 'a group that names us is the one we follow');
  const tie = parseRobots('User-agent: *\nDisallow: /jobs\nAllow: /jobs\nDisallow: /*?sort=\n');
  eq([allowed(tie, '/jobs'), allowed(tie, '/list?sort=date'), allowed(tie, '/robots.txt')], [true, false, true], 'an Allow as long as a Disallow wins, the query counts, and robots.txt itself may be read');
  eq(['es.indeed.com', 'www.reed.co.uk', 'www.jobs.ch', 'karriere.at', 'eures.europa.eu', 'talent.siemens.com', 'x.siemens.com', 'jobs.bosch.de', 'finnair.com'].map(isBoardHost), [true, true, true, true, true, false, false, false, false], 'job boards by the name they are registered under, not an employer host that starts with one');
  ...
  eq([pathShape('https://x.example/o/ict-medewerker-2-112'), pathShape('https://x.example/o/dotnet-angular'), pathShape('https://x.example/poste/0cl8xznbfe-it-asset-manager-hf/'), pathShape('https://x.example/senior-engineer-hamburg'), pathShape('https://x.example/'), pathShape('https://x.example/de/eine-lange-stellenbeschreibung'), pathShape('https://x.example/article/fault-managed-power-vs-ac')],
