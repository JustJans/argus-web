...
function compileDoubleArray(table) {
	...
	for (let id = 0; id < slots.length; id++) {
		...
		let edge = start;
		while (edge < end) if (check[offset + table.labels[edge] + 1] !== void 0) {
			offset++;
			edge = start;
		} else edge++;
	...
	for (let id = 0; id < slots.length; id++) terminals[slots[id] + 1] = table.terminals[id + 1] - table.terminals[id];
	for (let slot = 1; slot < terminals.length; slot++) terminals[slot] += terminals[slot - 1];
	...
	for (let id = 0; id < slots.length; id++) {
		...
		if (start !== end) patterns.set(table.patterns.subarray(start, end), terminals[slot]);
	}
	return {
		alphabet,
		bases,
...
function doubleArray(table) {
	...
	return {
		stats: {
			...
			typedArrayBytes: bases.byteLength + parents.byteLength + failures.byteLength + outputs.byteLength + terminals.byteLength + patterns.byteLength
		},
		advance(state, unit) {
			...
			while (true) {
				...
				if (parents[next] === state) return next;
				if (state === 0) return 0;
				state = failures[state];
			}
		},
		*outputs(state) {
			for (let output = state; output !== -1; output = outputs[output]) for (let index = terminals[output]; index < terminals[output + 1]; index++) yield patterns[index];
		}
	};
}
//#endregion
export { doubleArray as t };
